...
export default function GymSettings() {
    ...
    const [imageType, setImageType] = useState(settings.logoImage && settings.logoImage.startsWith("data:") ? "custom" : "icon");
    ...
    useEffect(() => {
        if (isLoaded) {
            ...
            setImageType(settings.logoImage && settings.logoImage.startsWith("data:") ? "custom" : "icon");
        ...
        <Card>
          ...
          <CardContent className="space-y-6">
            ...
            <div className="space-y-2">
              ...
              <Input id="gym-name" value={gymName} onChange={(e) => setGymName(e.target.value)} placeholder="e.g., JK GYM" data-testid="input-gym-name"/>
            </div>
            ...
            <div className="space-y-3">
              ...
              <div className="space-y-3">
                {/* File Input */}
                <div onClick={() => fileInputRef.current?.click()} className="border-2 border-dashed border-slate-300 dark:border-slate-600 rounded-lg p-6 text-center cursor-pointer hover:border-slate-400 dark:hover:border-slate-500 transition-colors duration-150" data-testid="dropzone-image-upload">
                ...
                {/* Image Preview */}
                {gymImage && imageType === "custom" && (<div className="space-y-3">
                    <div className="relative group">
                      <img src={gymImage} alt="Gym Logo Preview" className="w-full h-40 object-cover rounded-lg border border-slate-200 dark:border-slate-700" style={{ transform: `scale(${cropScale})` }} data-testid="image-preview"/>
                      <Button size="sm" variant="destructive" onClick={handleRemoveImage} className="absolute top-2 right-2 opacity-0 group-hover:opacity-100 transition-opacity duration-150" data-testid="button-remove-image">
                        <X className="w-4 h-4"/>
                      </Button>
                    </div>
                    ...
                    <div className="space-y-2">
                      ...
                      <input type="range" min="0.8" max="2" step="0.1" value={cropScale} onChange={(e) => setCropScale(parseFloat(e.target.value))} className="w-full" data-testid="slider-crop-scale"/>
              ...
              <div className="pt-4 space-y-3 border-t border-slate-200 dark:border-slate-700">
                ...
                <div className="grid grid-cols-4 gap-2">
                  {ICON_STYLES.map((style, index) => (<button key={index} onClick={() => handleSelectIcon(index)} className={`p-4 rounded-lg text-white font-semibold text-lg transition-all duration-200 border-2 ${selectedStyle === index && !gymImage
                ? "border-slate-900 dark:border-slate-100 ring-2 ring-slate-900 dark:ring-slate-100"
                : "border-transparent hover:opacity-80"} ${style.bg}`} data-testid={`button-icon-style-${index}`}>
                      {style.icon}
                    </button>))}
                </div>
              </div>
            ...
            <div className="space-y-3">
              ...
              <div className="flex items-center gap-4">
                <input type="color" value={primaryColor} onChange={(e) => setPrimaryColor(e.target.value)} className="w-16 h-12 rounded-lg cursor-pointer border-2 border-slate-300 dark:border-slate-600" data-testid="input-primary-color"/>
        ...
        <Card>
          ...
          <CardContent className="space-y-6">
            ...
            <div className="space-y-2">
              ...
              <Input id="owner-email" type="email" value={ownerEmail} onChange={(e) => setOwnerEmail(e.target.value)} placeholder="e.g., owner@gym.com" data-testid="input-owner-email"/>
            </div>
            ...
            <div className="space-y-2">
              ...
              <div className="relative">
                <Input id="owner-password" type={showOwnerPassword ? "text" : "password"} value={ownerPassword} onChange={(e) => setOwnerPassword(e.target.value)} placeholder="Enter a login password" data-testid="input-owner-password" className="pr-10"/>
                <button type="button" onClick={() => setShowOwnerPassword((v) => !v)} className="absolute inset-y-0 right-0 flex items-center pr-3 text-muted-foreground hover:text-foreground" aria-label={showOwnerPassword ? "Hide password" : "Show password"} data-testid="toggle-owner-password">
        ...
        <Card className="sticky top-6">
          ...
          <CardContent className="space-y-6">
              ...
              <div className="bg-white dark:bg-slate-950 border border-slate-200 dark:border-slate-800 rounded-lg p-4 space-y-3">
                <div className="flex items-center gap-3">
                  <div className="w-10 h-10 rounded-lg flex items-center justify-center text-white text-xl flex-shrink-0 overflow-hidden" style={{ backgroundColor: primaryColor }}>
                    {gymImage && imageType === "custom" ? (<img src={gymImage} alt="Logo" className="w-full h-full object-cover" style={{ transform: `scale(${cropScale})` }}/>) : (ICON_STYLES[selectedStyle].icon)}
                  </div>
                  <div>
            ...
            <div className="space-y-2">
              ...
              <Button className="w-full text-white font-semibold" style={{ backgroundColor: primaryColor }}>
                Sample Button
              </Button>
            </div>
