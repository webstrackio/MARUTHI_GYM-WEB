...
export default function AttendanceHistory() {
    ...
    const filteredRecords = attendanceRecords?.filter((record) => !searchRegister ||
    ...
    return (<div className="space-y-6">
      <div className="flex items-center justify-between">
        ...
        <Button onClick={handleExportCSV} variant="outline" disabled={!filteredRecords || filteredRecords.length === 0} data-testid="button-export-csv">
        ...
        <CardContent className="space-y-4">
          <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
            <div className="space-y-2">
              ...
              <Input type="date" value={selectedDate} onChange={(e) => setSelectedDate(e.target.value)} data-testid="input-select-date"/>
            </div>
            <div className="space-y-2">
              <label className="text-sm font-medium flex items-center gap-2">
              ...
              <Input placeholder="Enter register number..." value={searchRegister} onChange={(e) => setSearchRegister(e.target.value.replace(/[^A-Za-z0-9]/g, ""))} data-testid="input-search-register"/>
            </div>
          </div>

          {isLoading ? (<div className="space-y-3">
              {[...Array(5)].map((_, i) => (<Skeleton key={i} className="h-12 w-full"/>))}
            </div>) : filteredRecords && filteredRecords.length > 0 ? (<div className="rounded-md border">
              <Table>
                <TableHeader>
                  <TableRow>
